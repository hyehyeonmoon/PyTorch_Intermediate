...
class MnistDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x = self.data[idx] #MNIST의 경우, torch.Size([784])
        y = self.labels[idx]

        if self.flatten:
            x = x.view(-1) #torch.Size([784]), view(-1)은 아예 벡터로 길게 만들어버리는 것
...
def load_mnist(is_train=True, flatten=True):
    ...
    x = dataset.data.float() / 255. #torch.Size([60000, 28, 28])
    y = dataset.targets #torch.Size([60000])

    if flatten:
        x = x.view(x.size(0), -1) #torch.Size([60000, 784])
